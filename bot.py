...
app = Client("music-bot", api_id=config.API_ID, api_hash=config.API_HASH, bot_token=config.BOT_TOKEN)
calls = PyTgCalls(app)
queues = QueueManager()
...
async def start_handler(_: Client, message: Message) -> None:
    await message.reply_text("🎵 **Music Bot**\n\n`/play <song>`\n`/queue`\n`/pause` / `/resume`\n`/skip`\n`/stop`\n`/volume 0-200`\n`/ping`")
...
async def ping_handler(_: Client, message: Message) -> None:
    ...
    await reply.edit_text(f"🏓 **Pong:** `{(time.perf_counter() - started) * 1000:.0f} ms`")
...
async def play_handler(_: Client, message: Message) -> None:
    ...
    try:
        ...
        await status.edit_text(f"❌ Could not play source.\n`{type(exc).__name__}`")
