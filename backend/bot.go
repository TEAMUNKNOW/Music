...
func startTelegramBot() {
	...
	if token == "" { log.Println("[INFO] Telegram bot disabled: BOT_TOKEN is not configured"); return }
...
func telegramAPI(token, method string, payload any) error {
	...
	resp, err := (&http.Client{Timeout:20*time.Second}).Post("https://api.telegram.org/bot"+token+"/"+method,"application/json",bytes.NewReader(data))
...
func pollTelegram(token string) {
	...
	var offset int64
	for {
		values := url.Values{}; values.Set("timeout","25"); values.Set("offset",strconv.FormatInt(offset,10))
		...
		err = json.NewDecoder(resp.Body).Decode(&result); resp.Body.Close()
		if err != nil || !result.OK { time.Sleep(3*time.Second); continue }

		for _, update := range result.Result {
			if update.UpdateID >= offset { offset = update.UpdateID + 1 }
			if update.Message == nil { continue }
			text := strings.TrimSpace(update.Message.Text); if text == "" { continue }
			fields := strings.Fields(text); if len(fields)==0 { continue }
			cmd := strings.ToLower(fields[0]); if i:=strings.Index(cmd,"@"); i>=0 { cmd=cmd[:i] }
			...
			case "/music":
				if os.Getenv("WEBAPP_URL")=="" { sendBotMessage(token,update.Message.Chat.ID,"Music Mini App URL is not configured yet.",nil) } else { sendBotMessage(token,update.Message.Chat.ID,"Tap below to open Music 🎵",botKeyboard()) }
			}
		}
	}
}
