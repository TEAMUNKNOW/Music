...
func Search(c *fiber.Ctx) error {
	q := c.Query("q")
	if q == "" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error":"query required"}) }
	tracks, err := search.Search(q)
	if err != nil { return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"error":"search failed", "details":err.Error()}) }
...
func GetTrack(c *fiber.Ctx) error {
	...
	info, err := search.ResolveStream(id, c.Query("source"))
	if err != nil { return c.Status(502).JSON(fiber.Map{"error":"failed to resolve stream", "details":err.Error()}) }
	if redisClient != nil { _ = redisClient.Set(context.Background(), "src:"+id, info.URL, 5*time.Minute).Err() }
	baseURL := os.Getenv("BASE_URL")
	if baseURL == "" { baseURL = "http://localhost:3000" }
	return c.JSON(fiber.Map{"id":id,"title":info.Title,"artist":info.Artist,"duration":info.Duration,"thumbnail":info.Thumbnail,"quality":info.Quality,"streamUrl":stream.GenerateSignedURL(id, baseURL),"expiresIn":90})
}

func Stream(c *fiber.Ctx) error {
	id, exp, sig := c.Params("id"), c.Query("exp"), c.Query("sig")
	if !stream.ValidateSignedRequest(id, exp, sig) { return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error":"invalid or expired signature"}) }
...
func WebSocketUpgrade(c *fiber.Ctx) error {
	if websocket.IsWebSocketUpgrade(c) { return websocket.New(handleWS)(c) }
	return fiber.ErrUpgradeRequired
}

func handleWS(conn *websocket.Conn) {
	defer conn.Close()
	roomID, userID := conn.Query("room"), conn.Query("user")
