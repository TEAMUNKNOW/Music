...
class MusicPlayer:
    ...
    async def play_track(self, chat_id: int, track: Track) -> None:
        await self.calls.play(chat_id, MediaStream(track.stream_url, video_flags=MediaStream.Flags.IGNORE))
        ...
        async with self._lock(chat_id):
            ...
            if state.current is None:
                ...
                try:
                    ...
                    state.current = None
                    raise
            return False, await self.queues.enqueue(chat_id, track)
        ...
        async with self._lock(chat_id):
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("Skip fallback failed in %s", chat_id)
                    state.current = None
